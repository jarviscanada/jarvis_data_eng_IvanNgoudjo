...
# 3. Parse hardware specifications
hostname=$(hostname -f)
lscpu_out=$(lscpu)
...
# Extraire la mémoire totale en kilo-octets (kB) depuis /proc/meminfo
total_mem=$(cat /proc/meminfo | egrep "^MemTotal:" | awk '{print $2}' | xargs)

# Timestamp au format UTC 'YYYY-MM-DD HH:MM:SS'
timestamp=$(date -u "+%Y-%m-%d %H:%M:%S")

# 4. Construct the INSERT statement
